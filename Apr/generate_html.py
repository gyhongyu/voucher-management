...
base_dir = r"E:\Projects\Voucher management\Apr"
pr_dir = os.path.join(base_dir, "Public Relations")
trans_dir = os.path.join(base_dir, "Transportation")

def parse_filename(filepath):
    filename = os.path.basename(filepath)
    match = re.match(r'^(\d{2})-([A-Za-z]{3})-(\d{2})-(.+)-(\d+)\.(pdf|jpg)$', filename, re.IGNORECASE)
    if match:
        day, month_str, year, desc, amount, ext = match.groups()
        months = {'Jan':'01', 'Feb':'02', 'Mar':'03', 'Apr':'04', 'May':'05', 'Jun':'06',
                  'Jul':'07', 'Aug':'08', 'Sep':'09', 'Oct':'10', 'Nov':'11', 'Dec':'12'}
        month = months.get(month_str.capitalize(), '04')
...
<body x-data="reimbursementApp()" @mouseup="clearTimer()" @touchend="clearTimer()">
    <div class="container">
        <div class="header">
            <h1>Voucher Manager <span style="font-size: 1rem; color: var(--text-dim); font-weight: normal;">April 2026</span></h1>
    ...
    <script>
        function reimbursementApp() {{
            const STORAGE_KEY = 'vouchers_data_apr_2026';
            ...
            if (saved) {{
                ...
                // 如果有新檔案被合併或舊檔案被刪除，就自動把新的結果覆寫回 localStorage
                if (isUpdated) {{
                    localStorage.setItem(STORAGE_KEY, JSON.stringify(initialData));
                    console.log('Detected new voucher files! They have been safely appended to your existing cache.');
                }}
            }} else {{
            ...
            return {{
                ...
                exportData() {{
                    ...
                    a.href = url;
                    a.download = `Apr_Vouchers_${{new Date().toISOString().slice(0,10)}}.json`;
                    a.click();
...
html_path = os.path.join(base_dir, "Reimbursement.html")
with open(html_path, "w", encoding="utf-8") as f:
    f.write(html_content)

print(f"Generated {html_path} with {len(pr_items)} PR items and {len(trans_items)} Trans items.")
